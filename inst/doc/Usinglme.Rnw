...
Both \code{SAS PROC MIXED} and \code{lmer} allow a mixed-effects model to
...
likelihood or \textsf{REML}.  This is the default criterion in \code{SAS
  PROC MIXED}.  The default criterion in \code{lmer} is maximum
likelihood.  To get \textsf{REML} estimates in \code{lmer}, set the
...
worth repeating.  SAS defaults to \textsf{REML} fits; \code{lmer}
